...
interface ChatProjectsState {
  ...
  readonly removeSource: (environmentId: EnvironmentId, projectId: string, sourceId: string) => void;
...
function sanitizeProject(value: unknown): ChatProject | null {
  ...
  const sources = Array.isArray(value.sources)
    ? value.sources.map(sanitizeSource).filter((source): source is ChatProjectSource => source !== null)
    : [];
  const threadIds = Array.isArray(value.threadIds)
    ? [...new Set(value.threadIds.filter((threadId): threadId is string => typeof threadId === "string"))]
    : [];
  return {
    id,
    name,
...
> {
  ...
  try {
    const parsed: unknown = JSON.parse(window.localStorage.getItem(CHAT_PROJECTS_STORAGE_KEY) ?? "{}");
...
export const useChatProjectsStore = create<ChatProjectsState>((set) => ({
  ...
  deleteProject: (environmentId, projectId) => {
    set((state) => {
      ...
      const activeProjectIdByEnvironment = { ...nextState.activeProjectIdByEnvironment, [environmentId]: null };

...
describe("compressImageForStash", () => {
  ...
  it("reports too-large when even the smallest encoding overflows the budget", async () => {
    // Keep the fixture small: the test only needs every encoded attempt to
    // exceed the budget, not multi-megabyte allocations for each pass.
    const budgetChars = 100;
    const { close } = stubCanvasPipeline(() => 1_000);

    const result = await compressImageForStash(makeFile(200_000), budgetChars);

...
// Resolving an executable by bare name walks every PATH entry for every spawn.
// Supervision therefore requires a known absolute ps path before polling begins.
const resolvePosixPsCommand = Effect.fn("terminal.resolvePosixPsCommand")(function* () {
  ...
  return yield* new TerminalSubprocessCheckError({
    command: "ps",
    cause: new Error("No supported absolute ps executable was found."),
  });
});
...
> {
  ...
  if (result.code !== 0 || result.timedOut || result.stdoutTruncated) {
    // A partial table is not authoritative; failing the snapshot preserves
    // each terminal's last known process metadata for the next tick.
    return yield* new TerminalSubprocessCheckError({
      command: "ps",
...
const windowsProcessTableSnapshot = Effect.fn("terminal.windowsProcessTableSnapshot")(
  ...
  > {
    ...
    const result = yield* processRunner
      .run({
        // Use the executable directly so cmd.exe cannot re-tokenize the
        // PowerShell pipeline before PowerShell receives it.
...
function shouldStripCsiSequence(body: string, finalByte: string): boolean {
  ...
  // DECRQM mode queries (...$p) and DECRPM replies (...$y) are terminal
  // protocol traffic. The `$` guard keeps setters such as DECSTR (!p) and
  // DECSCL ("p) intact.
  if ((finalByte === "p" || finalByte === "y") && /^[0-9;?]*\$$/.test(body)) {
    return true;
  }
  // XTVERSION query (>q). DECSCUSR (space-intermediate q) stays intact.
...
// have no visual value and replaying them triggers fresh terminal responses.
...
export const makeWithOptions = Effect.fn("TerminalManager.makeWithOptions")(function* (
  ...
  const processRunner = yield* ProcessRunner.ProcessRunner;
  // Acquire one process-table snapshot per poll tick and share it across all
  // terminals. Per-terminal process commands scale with session count and can
  // exhaust host process capacity during long-running desktop use.

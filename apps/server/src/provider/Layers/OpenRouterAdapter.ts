...
export const makeOpenRouterAdapter = (config: {
  ...
  Effect.gen(function* () {
    const events = yield* Effect.acquireRelease(PubSub.unbounded<ProviderRuntimeEvent>(), PubSub.shutdown);
    ...
    const adapter: ProviderAdapterShape<ProviderAdapterError> = {
      ...
      startSession: (input) =>
        Effect.gen(function* () {
          ...
          yield* publish({ type: "session.started", ...stamp(now), provider: PROVIDER, providerInstanceId: input.providerInstanceId, threadId: input.threadId, payload: {} });
          yield* publish({ type: "session.state.changed", ...stamp(now), provider: PROVIDER, providerInstanceId: input.providerInstanceId, threadId: input.threadId, payload: { state: "ready", reason: "OpenRouter session ready" } });
          yield* publish({ type: "thread.started", ...stamp(now), provider: PROVIDER, providerInstanceId: input.providerInstanceId, threadId: input.threadId, payload: { providerThreadId: input.threadId } });
          return session;
        }),
      sendTurn: (input) =>
        Effect.gen(function* () {
          ...
          if (state.session.status === "closed") return yield* new ProviderAdapterSessionClosedError({ provider: PROVIDER, threadId: input.threadId });
          if (!input.input || input.input.trim().length === 0) return yield* new ProviderAdapterValidationError({ provider: PROVIDER, operation: "sendTurn", issue: "OpenRouter requires text input." });
          if ((input.attachments?.length ?? 0) > 0) return yield* new ProviderAdapterValidationError({ provider: PROVIDER, operation: "sendTurn", issue: "The direct OpenRouter driver currently accepts text turns only." });
          const now = DateTime.formatIso(yield* DateTime.now);
          ...
          state.session = { ...state.session, status: "running", activeTurnId: turnId, model, updatedAt: now };
          sessions.set(input.threadId, state);
          yield* publish({ type: "turn.started", ...stamp(now), provider: PROVIDER, threadId: input.threadId, turnId, payload: { model } });
          const itemId = RuntimeItemId.make(`openrouter-item-${turnId}`);
          yield* publish({ type: "item.started", ...stamp(now), provider: PROVIDER, threadId: input.threadId, turnId, itemId, payload: { itemType: "assistant_message", status: "inProgress" } });
      ...
      interruptTurn: (threadId, turnId) =>
        Effect.gen(function* () {
          ...
          if (activeTurnId) {
            ...
            yield* publish({ type: "turn.aborted", ...stamp(state.session.updatedAt), provider: PROVIDER, threadId, turnId: activeTurnId, payload: { reason: "Interrupted by user" } });
          }
        }),
      respondToRequest: (_threadId: ThreadId, _requestId: ApprovalRequestId, _decision: ProviderApprovalDecision) => Effect.fail(new ProviderAdapterRequestError({ provider: PROVIDER, method: "respondToRequest", detail: "OpenRouter does not expose approval requests." })),
      respondToUserInput: (_threadId: ThreadId, _requestId: ApprovalRequestId, _answers: ProviderUserInputAnswers) => Effect.fail(new ProviderAdapterRequestError({ provider: PROVIDER, method: "respondToUserInput", detail: "OpenRouter does not expose user-input requests." })),
      stopSession: (threadId) =>
        Effect.gen(function* () {
          ...
          const { activeTurnId: _activeTurnId, ...sessionWithoutActiveTurn } = state.session;
          state.session = { ...sessionWithoutActiveTurn, status: "closed", updatedAt: DateTime.formatIso(yield* DateTime.now) };
          sessions.set(threadId, state);
          yield* publish({ type: "session.exited", ...stamp(DateTime.formatIso(yield* DateTime.now)), provider: PROVIDER, threadId, payload: { reason: "Stopped by user", recoverable: true, exitKind: "graceful" } });
        }),
      ...
      readThread: (threadId) => {
        ...
        for (let index = 0; index < state.messages.length; index += 2) {
          turns.push({ id: TurnId.make(`openrouter-history-${index}`), items: state.messages.slice(index, index + 2) });
        }
        return Effect.succeed({ threadId: ThreadId.make(threadId), turns } satisfies ProviderThreadSnapshot);
      },
      ...
      stopAll: () => Effect.forEach([...sessions.keys()], (threadId) => adapter.stopSession(ThreadId.make(threadId)), { discard: true }),

...
describe("ProviderSessionReaper", () => {
  ...
  it.each(["older", "missing", "invalid"] as const)(
    "uses the binding idle window when the session timestamp is %s",
    async (sessionTimestamp) => {
            ...
            session:
              sessionTimestamp !== "missing"
                ? {
                    ...
                    lastError: null,
                    updatedAt:
                      sessionTimestamp === "invalid" ? "invalid" : "2026-04-14T00:00:00.000Z",
                  }
                : null,
          },
        ]),

...
const discoverDroidInventory = (
  ...
  options?: {
    readonly inventorySpawnOverride?: {
  ...
  Effect.gen(function* () {
    const rpc = yield* makeDroidExecRpcClient({
      ...
      cwd,
      env: environment,
      ...(options?.inventorySpawnOverride ? { spawnOverride: options.inventorySpawnOverride } : {}),
    });
...
> {
  ...
  const discoveryExit = yield* discoverDroidInventory(
    droidSettings,
    environment,
    cwd,
    options,
  ).pipe(Effect.timeoutOption(INVENTORY_DISCOVERY_TIMEOUT_MS), Effect.exit);
  const inventory =

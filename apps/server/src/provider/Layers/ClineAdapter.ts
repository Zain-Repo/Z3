...
const CLINE_PLAN_MODE_ALIASES = ["plan"];
const CLINE_IMPLEMENT_MODE_ALIASES = ["act", "code", "agent", "default", "chat", "implement"];
...
function isPlanMode(mode: AcpSessionMode): boolean {
  return findModeByAliases([mode], CLINE_PLAN_MODE_ALIASES) !== undefined;
}
...
function resolveRequestedModeId(input: {
  ...
  if (input.interactionMode === "plan") {
    return findModeByAliases(modeState.availableModes, CLINE_PLAN_MODE_ALIASES)?.id;
  }

  return (
    findModeByAliases(modeState.availableModes, CLINE_IMPLEMENT_MODE_ALIASES)?.id ??
...
/**
 * Apply the requested model and mode for a session or turn. Cline advertises
 * exactly "plan" and "act" modes; model selection is a no-op when the
 * requested id already matches the session's current model.
 */
function applyRequestedSessionConfiguration<E>(input: {
  ...
  return Effect.gen(function* () {
    const boundModelId = yield* applyClineAcpModelSelection({
...
export function makeClineAdapter(clineSettings: ClineSettings, options?: ClineAdapterLiveOptions) {
  return Effect.gen(function* () {
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process Cline ACP callback.",
              cause,
            }),
        ),
      );
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.cline.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.cline.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
        const fingerprint =
          (ctx.activeTurnId ?? "no-turn") +
          ":" +
          (encodeJsonStringForDiagnostics(payload) ?? "[unserializable payload]");
    ...
    const startSession: ClineAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const requestedStartModelId = clineModelSelection?.model
            ? resolveClineAcpBaseModelId(clineModelSelection.model)
            : undefined;
      ...
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          const requestedTurnModelId = turnModelSelection?.model
            ? resolveClineAcpBaseModelId(turnModelSelection.model)
            : undefined;
          ...
          if (steeringTurnId === undefined) {
            yield* offerRuntimeEvent({
              ...
              threadId: input.threadId,
              turnId,
              ...(currentModelId ? { payload: { model: currentModelId } } : { payload: {} }),
            });
          }
            ...
            for (const attachment of input.attachments) {
              if (attachment.type !== "image") {
                return yield* new ProviderAdapterValidationError({
                  provider: PROVIDER,
                  operation: "sendTurn",
                  issue: "Cline accepts native image attachments only.",
                });
              }

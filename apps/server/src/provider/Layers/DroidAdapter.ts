...
export interface DroidAdapterLiveOptions {
  ...
  /** Internal test hook; production always launches the configured Droid CLI. */
  readonly rpcSpawnOverride?: DroidExecRpcInput["spawnOverride"];
 ...
 * must be one of the outcomes the request offered. Unknown, auto-run, and
 * new-session outcomes remain unsupported so a generic approval cannot grant
 * broader autonomy than the user selected.
 */
export function selectDroidPermissionOutcome(
  ...
  switch (decision) {
    case "accept":
      return outcomes.find((outcome) => outcome.trim() === "proceed_once");
    case "acceptForSession":
      return outcomes.find((outcome) => persistentOutcomes.has(outcome.trim()));
    case "decline":
...
export function makeDroidAdapter(droidSettings: DroidSettings, options?: DroidAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    options?.registerDebugStateReader?.((threadId) =>
      threadLocks.inspect(threadId).pipe(
        Effect.map((lockState) => ({
          threadLockCount: lockState.keyCount,
          threadLockReferenceCount: lockState.references,
          interruptedTurnCount: sessions.get(threadId)?.interruptedTurnIds.size ?? 0,
          pendingApprovalCount: sessions.get(threadId)?.pendingApprovals.size ?? 0,
          pendingUserInputCount: sessions.get(threadId)?.pendingUserInputs.size ?? 0,
        })),
      ),
    );
    ...
    const handleTurnCompleted = (
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const turnId = live.session.activeTurnId;
      ...
      Effect.gen(function* () {
        ...
        const turnId = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            ctx.pendingApprovals.set(requestId, { resolution, supportedDecisions });
            yield* offerRuntimeEvent({
              type: "request.opened",
              ...(yield* makeEventStamp()),
              provider: PROVIDER,
        ...
        const approvedSpecHandoff =
          ...
          (yield* withThreadLock(
            ctx.threadId,
            Effect.sync(() => {
              if (ctx.stopped || ctx.session.activeTurnId !== turnId) return false;
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const rpc = yield* makeDroidExecRpcClient({
            ...
            ...(options?.rpcSpawnOverride ? { spawnOverride: options.rpcSpawnOverride } : {}),
          }).pipe(
      ...
      Effect.gen(function* () {
        ...
        yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            const interruptExit = yield* requestViaRpc(ctx, "droid.interrupt_session", {}).pipe(
              Effect.exit,
            );
            if (ctx.pendingInterrupt === barrier) ctx.pendingInterrupt = undefined;
            yield* Deferred.succeed(barrier, undefined);
            ctx.interruptedTurnIds.delete(interruptedTurnId);
            if (Exit.isFailure(interruptExit)) {
              yield* settleTurn(ctx, interruptedTurnId, {
                state: "failed",
                errorMessage: "Droid could not confirm that the active turn was interrupted.",
              });
    ...
    const rollbackThreadToInternal = (threadId: ThreadId, target: ProviderThreadRollbackTarget) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
    ...
    const rollbackThreadTo = (threadId: ThreadId, target: ProviderThreadRollbackTarget) =>
      withThreadLock(threadId, rollbackThreadToInternal(threadId, target));
        ...
        Effect.gen(function* () {
          if (!Number.isInteger(numTurns) || numTurns < 1) {
            return yield* new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "rollbackThread",
              issue: "The number of turns to roll back must be at least one.",
            });
          }
          const ctx = yield* requireSession(threadId);
          const nextLength = Math.max(0, ctx.turns.length - numTurns);
          const anchorTurnId = ctx.turns[nextLength]?.id;
          return yield* rollbackThreadToInternal(threadId, {
            turnIds: ctx.turns.slice(0, nextLength).map((turn) => turn.id),

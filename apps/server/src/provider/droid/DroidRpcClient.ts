...
export interface DroidExecRpcInput {
  ...
  readonly cwd?: string;
  readonly env?: NodeJS.ProcessEnv;
  /** Internal test hook; production uses the Droid exec command below. */
  readonly spawnOverride?: Pick<DroidRpcSpawnInput, "command" | "args">;
}
...
> =>
  Effect.gen(function* () {
    ...
    // Lossless delivery must never suspend stdout response processing. A hard
    // backlog limit below turns an abandoned consumer into a diagnosed exit.
    ...
    const lifecycle = yield* Ref.make<DroidRpcLifecycle>({
      _tag: "Running",
      pending: new Map(),
    });
    ...
    const resolveResponse = (message: ParsedJsonRpcMessage): Effect.Effect<void> =>
      Effect.gen(function* () {
        ...
        const requestState = yield* Ref.modify(lifecycle, (state) => {
        ...
        if (backlog >= DROID_LOSSLESS_BACKLOG_HARD_LIMIT) {
          return yield* new DroidRpcError({
            ...
            data: {
              ...
              backlog,
              limit: DROID_LOSSLESS_BACKLOG_HARD_LIMIT,
            },
          });
        }
    ...
    const finalizeTransportExit = (exit: DroidProcessExit) =>
      Effect.gen(function* () {
        ...
        yield* Effect.all(
          [Queue.shutdown(notificationPubSub), Queue.shutdown(serverRequestPubSub)],
          {
            discard: true,
          },
        );
    ...
    const request: DroidRpcClient["request"] = (method, params, options) =>
      Effect.gen(function* () {
        ...
        const exited = yield* Ref.modify(lifecycle, (state) => {
          if (state._tag === "Running") {
            const next = new Map(state.pending);
            next.set(requestId, { _tag: "Pending", method, deferred, sent: false });
            return [undefined, { ...state, pending: next }] as const;
          }
          const exit =
            state._tag === "Exited"
              ? state.exit
          ...
          return [exit, state] as const;
        });
        if (exited !== undefined) {
          return yield* Effect.fail(processExitError(exited, method));
        }
          ...
          timeoutMs === undefined
            ...
            : exchange.pipe(
                ...
                Effect.flatMap((result) => {
                  ...
                  return Ref.modify(lifecycle, (state) => {
                    if (state._tag === "Exited") {
                      return [false, state] as const;
                    }
                    const entry = state.pending.get(requestId);
    ...
    const shutdown = Ref.modify(lifecycle, (state) => {
      if (state._tag !== "Running") {
        return [false, state] as const;
      }
      return [true, { _tag: "ShuttingDown", pending: state.pending }] as const;
    }).pipe(
    ...
    return {
      request,
      notifications: Stream.fromQueue(notificationPubSub).pipe(
        Stream.catchCause(() => Stream.empty),
      ),
      serverRequests: Stream.fromQueue(serverRequestPubSub).pipe(
        Stream.catchCause(() => Stream.empty),
      ),
      exits,
      shutdown,

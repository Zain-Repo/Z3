  ...
  return {
    ...
    providers: {
      ...
      grok: {
        ...
        enabled:
          persisted.providers?.grok?.enabled ??
          (usedProviders.has("grok") ? true : settings.providers.grok.enabled),
      },
      opencode: {
        ...settings.providers.opencode,
        enabled:
          persisted.providers?.opencode?.enabled ??
          (usedProviders.has("opencode") ? true : settings.providers.opencode.enabled),
      },
    },
    providerInstances,
  };
...
// Optional providers are enabled by history on load, so their persisted
// defaults must remain distinguishable from an explicit user choice.

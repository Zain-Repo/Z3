...
> For maintainers. Using Z3? See [docs/user](../user/).
...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates Z3, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Z3 review diff.
...
The Pending rows use the production offline outbox and point at the real Z3 and React fixture

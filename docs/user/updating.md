...
The Z3 web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Z3 shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                  |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Z3 Linux background service. Select the button and leave Z3 open while it prepares, tests, restarts, and reconnects.                            |
| **Update the desktop app** | Open the Z3 desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Z3 server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Z3 does not update connected
...
If the server instead runs as the Z3 background service, update the service on the host and
...
See [Running Z3 in the Background](./background-service.md) for install, status, and removal
commands.

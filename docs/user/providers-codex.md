...
This guide is for people who want to use more than one Codex account in Z3. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Z3](./install.md).
...
In Z3 Settings, name it something obvious:
...
In Z3 Settings, add another Codex provider:
...
Z3 shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Z3 treats it

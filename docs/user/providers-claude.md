...
This guide is for people who want to use more than one Claude setup in Z3. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Z3](./install.md).
...
In Z3 Settings, your Claude provider can stay like this:
...
When you set `CLAUDE_CONFIG_DIR path`, Z3 points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Z3 looks.

Then add another Claude provider in Z3:
...
Z3 only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Z3 keeps separate config directories isolated
...
Add or edit a Claude provider in Z3 Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Z3 stores the value as a server secret and does not
...
Z3 does not need a special Claude Code Router provider. Treat the router as a Claude

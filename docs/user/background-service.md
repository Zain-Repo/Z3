# Running Z3 in the Background

On a Linux host, Z3 can run as a background service for your user. It starts when the machine
boots and keeps running after you log out.

## Manage the Service

Install it with the latest Z3 release:
...
Updating restarts Z3 briefly. Let active agent work and terminal commands finish first.

The systemd unit runs a small stable launcher. Exact Z3 versions are installed separately, so
...
want Z3 to start in the background.
